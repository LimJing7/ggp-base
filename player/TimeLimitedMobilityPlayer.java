...
public class TimeLimitedMobilityPlayer extends OneTwoPlayer {
    ...
    public Move play(long timeout)
        ...
        if (this.onePlayer) {
            ...
            for (Move move : legalMoves) {
                ...
                Pair<Integer, Double> result = oneMaxScorePair(role, this.findNext(action, state, machine), timeout, this.searchDepth);
    ...
    protected Pair<Integer, Double> twoMinScorePair(Role role, Move move, MachineState state, double alpha, double beta, long timeout,int searchDepth)
            throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
    ...
    protected Pair<Integer, Double> twoMaxScorePair(Role role, MachineState state, double alpha, double beta, long timeout, int searchDepth)
            throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
        StateMachine machine = getStateMachine();
    ...
    protected Pair<Integer, Double> oneMaxScorePair(Role role, MachineState state, long timeout, int searchDepth) throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
        StateMachine machine = getStateMachine();
        if (this.findTerminalp(state, machine)) {
            return Pair.of(1, (double) this.findReward(role, state, machine));
        }
